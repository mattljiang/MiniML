...
module Env : Env_type =
  struct
    ...
    let lookup (env : env) (varname : varid) : value =
      ...
      with
      | Not_found -> let msg = "Unbound variable " ^ varname in
                     raise (EvalError msg) ;;
...
(*       with
    ...
    let rec value_to_string ?(printenvp : bool = true) (v : value) : string =
      ...
      | Closure (exp, env) ->
                      if printenvp
                      then exp_to_concrete_string exp ^ " START CLOSURE ENV: " ^
                           env_to_string env
...
let eval_s (exp : expr) (_env : Env.env) : Env.value =
  let rec eval_help (exp: expr) : expr =
    match exp with
    | Var x -> (*lookup env x*)
    let msg = "Unbound variable " ^ x in raise (EvalError msg)
    | Num n -> Num n
    | Bool b -> Bool b
    | Unop (u, e) -> (match eval_help e with
                      | Num n -> (match u with Negate -> Num (~-n))
                      | _ -> raise (EvalError "Invalid operation"))
    | Binop (b, e1, e2) -> (match eval_help e1, eval_help e2 with
                            ...
                            | Bool b1, Bool b2 -> (match b with
                              ...
                              | _ -> raise (EvalError "Invalid operation"))
                            | _ -> raise (EvalError "Invalid operation"))
    | Conditional (e1, e2, e3) -> (match eval_help e1 with
                                   | Bool b ->
                                        if b then eval_help e2
                                        else eval_help e3
                                   | _ -> raise (EvalError "Invalid condition"))
    (*should be this instead?*)
    (* | Fun (y, e) -> Fun (y, eval_help e) *)
    | Fun (y, e) -> Fun (y, e)
    | Let (y, def, body) -> eval_help (subst y (eval_help def) body)
    | Letrec (f, def, body) ->
                  let def' = eval_help (subst f (Letrec (f, def, Var f)) def) in
                  eval_help (subst f def' body)
    | Raise -> Raise
    | Unassigned -> Unassigned
    | App (p, q) ->
          eval_help (match eval_help p with
                     | Fun (y, e) -> subst y q e
