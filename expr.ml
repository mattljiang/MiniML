...
let rec subst (var_name : varid) (repl : expr) (exp : expr) : expr =
  ...
  (*what happens if there's more than one argument??*)
  ...
  | Letrec (f, def, body) ->
                if f = var_name then Letrec (f, def, body)
                  ...
                  then Letrec (f, subst var_name repl def, subst var_name repl body)
                else let z = new_varname () in
...
let rec exp_to_concrete_string (exp : expr) : string =
  let binop_help b e1 e2 : string =
   ...
   match exp with
    ...
    | Letrec (f, def, body) -> " let rec " ^ f ^ " = " ^ exp_to_concrete_string def ^
                            " in " ^ exp_to_concrete_string body
...
let rec exp_to_abstract_string (exp : expr) : string =
  let binop_help b e1 e2 : string =
    ...
    | Fun (y, e) -> "Fun (" ^ y ^ ", " ^ exp_to_abstract_string e                  (* function definitions *)
    ...
    | Letrec (f, def, body) -> "Letrec (" ^ f ^ ", " ^ exp_to_abstract_string def ^
                            ", " ^ exp_to_abstract_string body ^ ")"        (* recursive local naming *)
