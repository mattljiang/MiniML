...
let fv_twoargs : expr = str_to_exp ("let x = 3 in let y = x in f x y ;;") ;;
...
let let_test_3 : expr = str_to_exp ("let var1 = 12 + y + 9 in 12 + y + var1 + 5 ;;") ;;
let let_test_3_def : expr = str_to_exp ("12 + y ;;") ;;

let letrec_test : expr = str_to_exp ("let rec f = fun x -> if x = 0 then 1
                                                           else y * f * (x-1) in
                                      f 4 + y ;;") ;;
let letrec_test_2 : expr = str_to_exp ("let rec f = fun x -> if x = 0 then 1
                                                           else 12 * f * (x-1) in
                                      f 4 + 12 ;;") ;;
let letrec_test_3 : expr = str_to_exp ("let rec var2 = fun x -> if x = 0 then 1
                                                           else f * var2 * (x-1) in
                                      var2 4 + f ;;") ;;
...
let app_test_1 : expr = str_to_exp ("(fun x -> x + 9) ((fun x -> x + 9) 5) ;;") ;;
...
(* let subst_test (var_name : varid) (repl : string) (exp : string) (result : string) : bool Lazy.t =
  lazy (subst "var_name" (str_to_exp repl) (str_to_exp exp) = (str_to_exp result)) ;; *)

let tests =
[
(*should fail if repl has to be value*)
...
test "subst x 12 -x"    (lazy (subst "x" (Num 12) (Unop (Negate, x)) = (Unop (Negate, Num 12))));
test "subst x 12 x*y"    (lazy (subst "x" (Num 12) (Binop (Times, x, y)) = Binop (Times, Num 12, y)));

test "subst x 12 cond_test" (lazy (subst "x" (Num 12) cond_test = cond_test_1));
test "subst x true cond_test" (lazy (subst "x" (Bool true) cond_test = cond_test_2));
...
test "subst x 12+y let_test" (lazy (subst "x" let_test_3_def
                                                        let_test = let_test_3));
test "subst f 12 letrec_test" (lazy (subst "f" (Num 12) letrec_test = letrec_test));
test "subst y 12 letrec_test" (lazy (subst "y" (Num 12) letrec_test = letrec_test_2));
test "subst y f letrec_test" (lazy (subst "y" f letrec_test = letrec_test_3));

test "subst x 12 raise_test"    (lazy (subst "x" (Num 12) raise_test = raise_test_1));
test "subst x 12 unassigned_test"    (lazy (subst "x" (Num 12) unassigned_test = unassigned_test_1));

test "subst f func (f 5)" (lazy (subst "f" func (App (f, Num 5)) = (App (func, Num 5))));
test "subst f func (f (f 5))" (lazy (subst "f" func app_test = app_test_1));
test "subst x 12 (func 5)" (lazy (subst "x" (Num 12) (App (func, Num 5)) = (App (func, Num 5))));
test "subst x 12 (func (5+x))" (lazy (subst "x" (Num 12) app_test2 = app_test2_1));

test "subst x 12 func_twoargs" (lazy (subst "x" (Num 12) func_twoargs = func_twoargs));
test "subst z 12 func_twoargs" (lazy (subst "z" (Num 12) func_twoargs2 = func_twoargs2_1));

test "free_vars x" (lazy (free_vars x = vars_of_list ["x"]));
test "free_vars x*x" (lazy (free_vars (Binop (Times, x, x)) = vars_of_list ["x"]));
...
test "free_vars x*9" (lazy (free_vars (Binop (Times, x, Num 9)) = vars_of_list ["x"]));
test "free_vars x*y" (lazy (free_vars (Binop (Times, x, y)) = vars_of_list ["x"; "y"]));

test "free_vars cond_test" (lazy (free_vars cond_test = vars_of_list ["x"; "y"]));
...
test "free_vars fv_twoargs" (lazy (free_vars fv_twoargs = vars_of_list ["f"]))
